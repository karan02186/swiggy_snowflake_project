...
TRUNCATE TABLE CLEAN_SCH.RESTAURANT;
TRUNCATE TABLE CONSUMPTION_SCH.RESTAURANT_DIM;
...
-- CREATING TABLE IN BRONZE LAYER SCHEMA(CLEAN SCHEMA)
...
-- TO CHECK ONLY FOR THE LINAGE
-- insert into CONSUMPTION_SCH.RESTAURANT_DIM (
--     RESTAURANT_HK,
--     restaurant_id,
--     name,
--     cuisine_type,
...
-- )
-- select 
--     hash(SHA1_hex(CONCAT('APPLE','BALL'))),
...
--     try_cast(location_id_fk as number) as location_id_fk,
...
--     -- _stg_file_md5
-- from 
--     CLEAN_SCH.restaurant;


DROP TABLE CONSUMPTION_SCH.RESTAURANT_DIM;
...
--- scd 2 (keeping historical records by adding end date and inserting the new records)
-- merge statement
-- MERGE BTW CLEAM RESTAUTRANT STREAM AND CONSUMPTION RESTAUTRANT TABLE
MERGE INTO 
...
file_format = (format_name = 'stage_sch.csv_file_format')
on_error = abort_statement;
-- FIRST EXECUTED THE QUERY FOR THE STAGE LOCATION DAY -01
--- Now Run the merge statements of scd1 and scd2
