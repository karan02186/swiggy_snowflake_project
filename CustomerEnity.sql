...
--merging cdc data to the target table
-- over writng the records when matched and updated(scd type 1)
...
SELECT * FROM CLEAN_SCH.CUSTOMER_STM
WHERE METADATA$ISUPDATE = 'TRUE';--- TRACKING CHANGES WHERE THE  RECORDS HAVE BEEN UPDATED (INSERT AND DELETE OPS)
-- create dim table 
...
-- merging data from silver layer to the gold layer
-- scdn type 2
-- maintiainig the version of effictive and active record
-- when value gets deleted, the record is set for end time and activity as false 
-- and then new version of the updated record is add with activity as true
...
-- now run both the merge statemets scd1 and scd2
...
-- AFTER COPY INTO CHECK  THE STAGE SCHEMA STM
SELECT * FROM STAGE_SCH.CUSTOMER_STM;
-- RUN THE FIRST MERGE STMT , THEN CHECK THE CLEAN SCHEMA STM ,
-- RUN THE MERGE STMT OF CONSUMPTION SCHEMA AND CHECK THE FINAL TABLE THAT SCHEMA
